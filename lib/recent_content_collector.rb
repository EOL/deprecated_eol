module RecentContentCollector
  def self.flickr_comments(hours = 24)
    ...
    # Also disregard comments which have an associated ActionsHistory - those comments will be sent
  ...
  def self.flickr_curator_actions(hours = 24)
    ...
    actions_history_ids = ActionsHistory.find(:all, :select => 'actions_histories.id', :joins => "JOIN #{DataObject.full_table_name} ON (actions_histories.changeable_object_type_id=#{ChangeableObjectType.data_object.id} AND actions_histories.object_id=data_objects.id)", :conditions => "actions_histories.created_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL #{hours} HOUR) AND actions_histories.action_with_object_id IN (#{ActionWithObject.trusted.id}, #{ActionWithObject.untrusted.id}, #{ActionWithObject.inappropriate.id}) AND data_objects.source_url LIKE 'http://www.flickr.com%'")
    actions_histories = ActionsHistory.find_all_by_id(actions_history_ids.collect{|ah| ah.id}, :include => [:user, :comment, :changeable_object_type, :action_with_object])
