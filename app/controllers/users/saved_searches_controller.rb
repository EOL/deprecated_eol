class Users::SavedSearchesController < UsersController
  ...
  def index
    # NOTE this #join avoids the problem where known_uri can be nil. Don't remove it unless you choose to clean that mess:
    @background_processes = DataSearchFile.where(user_id: @user.id).join(:known_uri).order('updated_at desc')
