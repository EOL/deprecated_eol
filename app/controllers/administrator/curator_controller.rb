class Administrator::CuratorController < AdminController
  ...
  def index
    ...
    search_string_parameters = ['%' + @user_search_string + '%'] * 6 # ...because there are six '?'s.

    @users = User.paginate(:conditions => [condition, *search_string_parameters],
      :order => 'curator_approved ASC, created_at DESC',:page => params[:page])
    @user_count = User.count(:conditions => [condition, *search_string_parameters])
  ...
  def export
    ...
    @users = User.find(:all,:conditions => ['curator_hierarchy_entry_id IS NOT NULL OR credentials != "" OR curator_scope!= ""'])
      ...
      CSV::Writer.generate(report, ',') do |title|
          title << ['Id', 'Username', 'Name', 'Email', 'Credentials','Scope','Clade','Approved','Registered Date','Objects Curated','Comments Moderated','Species Curated','Objects Shown','Objects Hidden','Objects Marked as Inappropriate','Text Data Objects Submitted']
          @users.each do |u|
            clade_name = u.curator_hierarchy_entry.nil? ? '' : u.curator_hierarchy_entry.name
            title << [u.id,u.username,u.full_name,u.email,u.credentials.gsub(/\r\n/,'; '),u.curator_scope.gsub(/\r\n/,'; '),clade_name,u.curator_approved,u.created_at,u.total_data_objects_curated,u.total_comments_curated,u.total_species_curated,u.total_objects_curated_by_action('show'),u.total_objects_curated_by_action('hide'),u.total_objects_curated_by_action('inappropriate'),UsersDataObject.count(:conditions => ['user_id = ?',u.id])]
          end
       end
       report.rewind
