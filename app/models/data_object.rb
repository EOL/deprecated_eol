...
class DataObject < ActiveRecord::Base
  ...
  def self.same_as_last?(params, options)
    ...
    user_data_object = UsersDataObject.find_by_data_object_id( last_dato.id )
    if user_data_object
      return  user_data_object.user_id == options[:user][:id] &&
              options[:taxon_concept][:id] == user_data_object.taxon_concept_id &&
