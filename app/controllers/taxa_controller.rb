class TaxaController < ApplicationController
  ...
  def show
    ...
    taxon_concept = @taxon_concept || find_taxon_concept || return
  ...
  # Changes the preferred common name in a selected language for a taxon concept.
  # Called from show method if action_name param is set to 'update_common_names'. Done in this way to
    ...
    if @taxon_concept.is_curatable_by?(current_user)
      ...
      if params[:trusted_name_clicked_on] != "false"
        if params[:trusted_name_checked] == "true"
          ...
          tcn = TaxonConceptName.find_by_synonym_id_and_taxon_concept_id(params[:trusted_synonym_clicked_on], @taxon_concept.id)
...
private
  ...
  def get_suggested_search_results(querystring)
    ...
    suggested_results.each do |res|
      common_name = suggested_results_original[res['taxon_concept_id'][0].to_s].taxon_concept.quick_common_name
