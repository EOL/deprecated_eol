module RecentContentCollector
  ...
  def self.flickr_curator_actions(hours = 24)
    ...
    curator_activity_log_ids = CuratorActivityLog.find(:all, :select => 'curator_activity_logs.id', :joins => "JOIN #{DataObject.full_table_name} ON (curator_activity_logs.changeable_object_type_id=#{ChangeableObjectType.data_object.id} AND curator_activity_logs.object_id=data_objects.id)", :conditions => "curator_activity_logs.created_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL #{hours} HOUR) AND curator_activity_logs.action_with_object_id IN (#{ActionWithObject.trusted.id}, #{ActionWithObject.untrusted.id}, #{ActionWithObject.inappropriate.id}) AND data_objects.source_url LIKE 'http://www.flickr.com%'")
    curator_activity_logs = CuratorActivityLog.find_all_by_id(curator_activity_log_ids.collect{|ah| ah.id}, :include => [:user, :comment, :changeable_object_type, :action_with_object])
