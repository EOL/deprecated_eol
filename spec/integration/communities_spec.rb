...
describe "Communities" do
  ...
  shared_examples_for 'all users' do
    ...
    context 'visiting index' do
      ...
      it 'should list all communities by name' do
        should have_tag('ul#communities_index li a', /#{@communities[:community].name}/)
        should have_tag('ul#communities_index li a', /#{@communities[:empty_community].name}/)
      end
    ...
    context 'visiting show community' do
      before(:all) { visit community_path(@communities[:community]) }
      subject { body }
      it 'should show the community name and description' do
        should have_tag('h1', /.*?#{@communities[:community].name}/)
        should have_tag('p#community_description', /.*?#{@communities[:community].description}/)
      end
      it 'should link to community roles' do
        body.should have_tag('ul#community_roles') do
          @communities[:community].roles.each do |role|
            with_tag("a[href=#{community_role_path(@communities[:community], role)}]", :text => /#{role.title}/m)
          end
        end
      end
      ...
      it 'should link to all of the community members' do
        body.should have_tag("ul#community_members") do
          @communities[:community].members.each do |member|
            user = member.user
      ...
      it 'should show the taxon concepts the community is focused upon' do
        body.should have_tag('#community_focus_container') do
          with_tag("a[href=#{taxon_concept_path(@communities[:tc1])}]")
          with_tag("a[href=#{taxon_concept_path(@communities[:tc2])}]")
        end
      end
    end
  end
  ...
  shared_examples_for 'logged in user' do
    ...
    context 'visiting create community' do
      ...
      it 'should create a community, add the user, and redirect to show on create' do
        ...
        click_button(@communities[:name_of_create_button])
    ...
    context 'visiting show community with feeds' do
      ...
      it 'should show feed items' do
        body.should have_tag('ul.feed') do
          with_tag('.feed_item .body', :text => @communities[:feed_body_1])
          with_tag('.feed_item .body', :text => @communities[:feed_body_2])
          with_tag('.feed_item .body', :text => @communities[:feed_body_3])
        end
      end
    end
    context 'visiting show community with empty feed' do
      before(:all) { visit community_path(@communities[:empty_community]) }
  ...
  shared_examples_for 'community member' do
    ...
    context 'visiting show community' do
      ...
      it 'should not show join community link' do
        should_not have_tag("a[href=?]", /#{join_community_path(@communities[:community].id)}.*/)
      end
      it 'should show leave community link' do
        should have_tag("a[href=?]", /#{leave_community_path(@communities[:community].id)}.*/)
      end
    end
  end
  ...
  describe 'anonymous user' do
    before(:all) do
      ...
      login_as @communities[:user_non_member]
      visit logout_path
    end
    it_should_behave_like 'all users'
    context 'visiting create community' do
    ...
    context 'visiting show community' do
      ...
      it 'should show a link to join the community' do
        body.should have_tag("#community_members_container") do
          with_tag("a[href=?]", /#{join_community_path(@communities[:community].id)}\?return_to=.*/)
        end
      end
    end
  end
  ...
  describe 'non member' do
    before(:all) { login_as @communities[:user_non_member] }
    it_should_behave_like 'logged in user'
    context 'visiting show community' do
      before(:all) { visit community_path(@communities[:community]) }
      subject { body }
      it 'should not show edit community links' do
        should_not have_tag("a[href=#{edit_community_path(@communities[:community])}]")
      end
      it 'should not show delete community links' do
        should_not have_tag("a[href=#{community_path(@communities[:community])}]", :text => /delete/i)
      end
      it 'should allow user to join community' do
        @communities[:user_non_member].member_of?(@communities[:community]).should_not be_true
        should have_tag("a[href=#{join_community_path(@communities[:community].id)}]")
        visit(join_community_path(@communities[:community].id))
        @communities[:user_non_member].reload
        @communities[:user_non_member].member_of?(@communities[:community]).should be_true
        body.should have_tag("ul#community_members") do
          with_tag("li.member a[href=#{user_path(@communities[:user_non_member])}]", :text => @communities[:user_non_member].username)
        end
        # Clean up - we have tested that a non member can join, we now make them leave
        @communities[:user_non_member].leave_community(@communities[:community])
        @communities[:user_non_member].member_of?(@communities[:community]).should_not be_true
      end
    end
    context 'visiting edit community' do
      it 'should not be allowed' do
        get edit_community_path(@communities[:community])
        response.should be_redirect
      end
    end
  end
  ...
  describe 'community member without community administration privileges' do
    before(:all) { login_as @communities[:user_community_member] }
    ...
    context 'visiting show community' do
      ...
      it 'should not show edit membership links' do
        should_not have_tag("a[href=#{community_member_path(@communities[:community], @communities[:community_member])}]", :text => /edit/i)
      end
      it 'should not show remove membership links' do
        should_not have_tag("a[href=#{community_member_path(@communities[:community], @communities[:community_member])}]", :text => /remove/i)
      end
      it 'should not show an add role link' do
        should_not have_tag("a[href=#{new_community_role_path(@communities[:community])}]")
      end
      it 'should allow member to leave community and return to show community' do
        @communities[:user_community_member].member_of?(@communities[:community]).should be_true
        visit(leave_community_path(@communities[:community].id))
        @communities[:user_community_member].reload
        @communities[:user_community_member].member_of?(@communities[:community]).should_not be_true
        body.should have_tag("ul#community_members") do
          without_tag("li.allowed a[href=#{user_path(@communities[:user_community_member])}]", :text => @communities[:user_community_member].username)
        end
        # Clean up - we have tested that a member can leave a community, now we make them join back
        @communities[:user_community_member].join_community(@communities[:community])
        @communities[:user_community_member].member_of?(@communities[:community]).should be_true
      end
    end
  end
  ...
  describe 'community member with community administration privileges' do
    before(:all) { login_as @communities[:user_community_administrator] }
    it_should_behave_like 'community member'
    ...
    context 'visiting show community' do
      ...
      it 'should show an add role link' do
        should have_tag("a[href=#{new_community_role_path(@communities[:community])}]")
      end
      it 'should show an edit community link' do
        should have_tag("a[href=#{edit_community_path(@communities[:community])}]")
      end
      it 'should show delete community link' do
        should have_tag("a[href=#{community_path(@communities[:community])}]", :text => /delete/i)
      end
      it 'should show edit membership links' do
        should have_tag("a[href=#{community_member_path(@communities[:community], @communities[:community_member])}]", :text => /edit/i)
      end
      it 'should show remove membership links' do
        should have_tag("a[href=#{community_member_path(@communities[:community], @communities[:community_member])}]", :text => /remove/i)
      end
    end
    context 'visiting edit community' do
      before(:all) { visit edit_community_path(@communities[:community]) }
      subject { body }
