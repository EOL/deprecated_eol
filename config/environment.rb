...
Rails::Initializer.run do |config|
  ...
  config.active_record.observers = :data_object_observer, :comment_observer
  ...
  $USE_SQL_SESSION_MANAGEMENT = false   # set to true to use Rails built-in SQL session storage management
   ...
   # CACHE CONFIGURATION
   $CACHE_CLEARED_LAST = Time.now()  # This variable will record the last time the cache was cleared
   $CACHE_CLEAR_IN_HOURS = 1 # automatically expire any non-species page cached fragments each hour
