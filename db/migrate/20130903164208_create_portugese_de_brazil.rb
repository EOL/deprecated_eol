...
class CreatePortugeseDeBrazil < ActiveRecord::Migration
  def up
    unless Language.exists?(iso_639_1: 'pt-BR')
      ...
      TranslatedLanguage.create(language_id: Language.default.id, original_language_id: l.id, label: 'Brazilian Portuguese')
      TranslatedLanguage.create(language_id: l.id, original_language_id: l.id, label: 'português do Brasil')
    end
  end
