/* I'm not enitrely sure what this does (other than updating references when the content area changes, but the function that
 * gets called from this lib is updateReferences. 
 *
...
function getElementsByClassForBioGuid(searchClass) {
...
// This is the function that we will call from elsewhere:
function updateReferences() {
	var imgholder = new Image();
	imgholder.src = bioGUIDicon[0];
	var aSpans = getElementsByClassForBioGuid(bioGUIDspanName);
	for(i=0;i<aSpans.length;i++){
		if ( aSpans[i].innerHTML.toLowerCase().indexOf('img') < 0 ) {
      // TODO - This could be cleaned up... less markup and more unobtrusive.
