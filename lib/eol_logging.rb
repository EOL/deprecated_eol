      ...
      (1..count.to_i).each do
        ::DataObjectLog.transaction do
          for n in 1..1000 do
            ...
            l = ::DataObjectLog.new
            l.ip_address_raw = ::IpAddress.ip2int addr
            l.user_agent = agent
            l.agent_id = agent_ids[n % agent_ids.size]
            l.created_at = (n * 15).minutes.ago
            # TODO Make this database agnostic.
            obj = ::DataObject.find(:first, :include => [:data_type], :conditions => [''], :order => 'RAND()', :limit => 1)
            l.data_object = obj
            l.data_type = obj.data_type
            if 0 == n % 4
              l.user_id = User.find_by_sql('SELECT id FROM users ORDER BY RAND() LIMIT 1')
            end
            l.save
          end
        end
