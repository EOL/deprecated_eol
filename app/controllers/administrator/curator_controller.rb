    ...
    @users = User.find(:all,:conditions => ['curator_hierarchy_entry_id IS NOT NULL OR credentials != "" OR curator_scope!= ""'])
      ...
      CSV::Writer.generate(report, ',') do |title|
          ...
          @users.each do |u|
            ...
            title << [u.id,u.username,u.full_name,u.email,u.credentials.gsub(/\r\n/,'; '),u.curator_scope.gsub(/\r\n/,'; '),clade_name,u.curator_approved,u.created_at,u.total_objects_curated,u.total_comments_curated,u.total_species_curated,u.total_objects_curated_by_action('show'),u.total_objects_curated_by_action('hide'),u.total_objects_curated_by_action('inappropriate'),UsersDataObject.count(:conditions => ['user_id = ?',u.id])]       
          end
       end
       report.rewind
