class RemovePublishingRelatedResourceStatuses < ActiveRecord::Migration
  def self.up
    status_ids_to_remove = TranslatedResourceStatus.find_all_by_label(['Published', 'Publish Pending', 'Unpublish Pending']).collect{|trs| trs.resource_status_id}.compact
  ...
  def self.down
    ...
    if english
      ['Published', 'Publish Pending', 'Unpublish Pending'].each do |status_label|
