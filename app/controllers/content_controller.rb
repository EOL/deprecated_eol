class ContentController < ApplicationController
  ...
  def mediarss
    ...
    if !taxon_concept.nil?
      ...
      rows = SpeciesSchemaModel.connection.execute("SELECT he.taxon_concept_id, do.object_cache_url, do.object_title, do.guid, do.data_rating FROM top_concept_images tci JOIN data_objects do ON (ti.data_object_id=do.id) WHERE tci.taxon_concept_id=#{taxon_concept.id} AND ti.view_order<400").all_hashes.uniq
