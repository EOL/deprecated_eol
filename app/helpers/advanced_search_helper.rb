module AdvancedSearchHelper
  ...
  def data_search_file_summary(search_file)
    ...
    unless search_file.q.blank?
      summary_parts << I18n.t('helpers.label.advanced_search.q_with_val', val: search_file.q)
    end
    if search_file.taxon_concept
      summary_parts << I18n.t('helpers.label.advanced_search.taxon_name_with_val',
    ...
    unless search_file.from.nil?
      summary_parts << I18n.t('helpers.label.advanced_search.min_with_val',
                val: display_text_for_data_point_uri(search_file.from_as_data_point))
    end
    unless search_file.to.nil?
      summary_parts << I18n.t('helpers.label.advanced_search.max_with_val',
                val: display_text_for_data_point_uri(search_file.to_as_data_point))
    end
    if search_file.complete?
      summary_parts << I18n.t('helpers.label.advanced_search.total_results',
