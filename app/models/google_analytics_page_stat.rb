class GoogleAnalyticsPageStat < ActiveRecord::Base
  ...
  def self.page_summary(user_id, year, month, page)      
    ...
    Where google_analytics_partner_taxa.user_id = ? AND
    google_analytics_partner_taxa.`year` = ? AND
    google_analytics_partner_taxa.`month` = ?
    Group By
    google_analytics_partner_taxa.taxon_concept_id,
    google_analytics_partner_taxa.user_id,
    google_analytics_partner_taxa.`year`,
    google_analytics_partner_taxa.`month`
    order by page_views desc "      
    self.paginate_by_sql [query, user_id, year, month], :page => page, :per_page => 50 , :order => 'page_views'
   end
end
