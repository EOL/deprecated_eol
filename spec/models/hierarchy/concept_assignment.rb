class Hierarchy
  class ConceptAssignment
    ...
    def compare_hierarchies(h1, h2)
      ...
      options = { sort: "relationship asc, visibility_id_1 asc, "\
      ...
      begin
        page ||= 0
        page += 1
        # TODO: make method, don't use connection:
        ...
        response["response"]["docs"].each do |entry|
          ...
          # "confidence"=>1.0 }
          # TODO: move to query:
          next if entry["relationship"] == 'syn' && entry["confidence"] < 0.25
          # TODO: I hate this kind of reassignment. :\
          ...
          next if hierarchy1.complete? && entries_matched.include?(id2)
          next if hierarchy2.complete? && entries_matched.include?(id1)
          ...
          # TODO: method has too many args TODO: rename
          ...
          # TODO - log the supercedure somewhere to be cleaned up, e.g.: in
    ...
    def confirmed_exclusions_matches?(id, other_tc_id)
      # TODO: what's equivalent ?
      @confirmed_exclusions[id1].each do |tc_id, equivalent|
