  ...
  describe '#data_search_results_summary' do
    ...
    context 'with results for "foo" and a taxon filter' do
      ...
      before do
        ...
        allow(@taxon_concept).to receive(:traits_searchable?) { true }
      end
  ...
  describe '#data_search_file_summary' do
    ...
    context "when everything is there" do
      ...
      let(:summary) do
        ...
        allow(helper).to receive(:display_text_for_trait).with(245) { 245 } # - BAD SMELL
        allow(helper).to receive(:display_text_for_trait).with(489) { 489 } # - BAD SMELL
