...
Things you need to do:
	...
	12. Run EOL Private Config
	13. Run the EOL Rails Tests
	14. Starting Your EOL Rails Server
...
while to rebuild from scratch.  However, the following commands should do the 
trick:
...
Once it is installed, there are some additional recommended steps:
...
template to start with:
	...
	cp $ROOT/config/database.sample.yml $ROOT/config/database.yml
...
We currently use an older version of therubyracer gem which indirectly depends
on an old version of the C++ compiler.  As result you will need to build just
this gem first.  In a new shell, run these commands:
...
Make sure you get rid of this old shell before you move on.

Finally, you should be able install all the rest of the needed gems with:
...
The foreground option is optional.
...
Next go to http://localhost:8890/ => Conductor => LinkedData => Graphs and 
check to see if there is a graph named http://localhost:8890%2FDAV%2Fxx%2Fyy 
...if so, delete it.
...
Java, and the required JAR file is included with the EOL project's source code.
