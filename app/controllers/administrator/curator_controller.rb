class Administrator::CuratorController < AdminController
  ...
  def export
    ...
    user_exemplar_images = EOL::Curator.total_objects_curated_by_action_and_user(Activity.choose_exemplar_image.id)
    user_exemplar_text = EOL::Curator.total_objects_curated_by_action_and_user(Activity.choose_exemplar_article.id)
    user_preferred_classifications = EOL::Curator.total_objects_curated_by_action_and_user(Activity.preferred_classification.id, nil, [ChangeableObjectType.curated_taxon_concept_preferred_entry.id])
    user_common_names_added = EOL::Curator.total_objects_curated_by_action_and_user(Activity.add_common_name.id, nil, [ChangeableObjectType.synonym.id])
    user_common_names_curated = EOL::Curator.total_objects_curated_by_action_and_user([Activity.trust_common_name.id, Activity.untrust_common_name.id, Activity.unreview_common_name.id, Activity.inappropriate_common_name.id], nil, [ChangeableObjectType.synonym.id])
