module RecentContentCollector
  ...
  def self.flickr_curator_actions(hours = 24)
    ...
    curator_activity_log_ids = CuratorActivityLog.find(:all, :select => 'curator_activity_logs.id', :joins => "JOIN #{DataObject.full_table_name} ON (curator_activity_logs.changeable_object_type_id IN (#{ChangeableObjectType.data_object.id}, #{ChangeableObjectType.data_objects_hierarchy_entry.id}) AND curator_activity_logs.object_id=data_objects.id)", :conditions => "curator_activity_logs.created_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL #{hours} HOUR) AND curator_activity_logs.activity_id IN (#{Activity.trusted.id}, #{Activity.untrusted.id}, #{Activity.inappropriate.id}) AND data_objects.source_url LIKE 'http://www.flickr.com%'")
