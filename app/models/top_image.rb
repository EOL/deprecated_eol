...
class TopImage < ActiveRecord::Base
  ...
  class << self
    ...
    def build_species_table_from(source, target)
      EOL::Db.truncate_table(connection, target)
      connection.execute("INSERT INTO #{target} "\
    ...
    def bulk_insert_tmp_images(top_images, top_unpublished_images)
      EOL.log_call
    ...
    def rebuild
      ...
      baseline_concept_images.each do |tc_id, datos|
        ...
        datos.each do |dato_id, val|
          ...
          baseline_hierarchy_entry_ids.each do |key, hierarchy_entry_id|
            ...
            which[hierarchy_entry_id][vetted_view_order][data_rating][data_object_id] =
              "#{hierarchy_entry_id}, #{data_object_id}"
          end
        end
      end
      EOL::Db.with_tmp_tables([TopImage, TopUnpublishedImage]) do
        ...
        parent_ids = Set.new
        HierarchyEntry.select(:parent_id).
          joins("JOIN top_images_tmp ti ON (he.id = ti.hierarchy_entry_id)").
          where("parent_id != 0").find_each do |he|
          parent_ids << he.parent_id
        end
        more_parents = find_best_images_in_clade(parent_ids)
        until(more_parents.empty?)
    ...
    def find_best_images_in_clade(parent_ids)
      ...
      parent_ids.to_a.in_groups_of(5000) do |group|
        ...
        results = HierarchyEntry.connection.select_all(
          "(SELECT he.id hierarchy_entry_id, he.parent_id, do.id,
          ...
          (SELECT he.id hierarchy_entry_id, he.parent_id, do.id,
            do.data_rating, dohe.visibility_id, dohe.vetted_id, do.published
              ...
              JOIN data_objects_hierarchy_entries dohe
                ON (do.id=dohe.data_object_id)
              WHERE he.id IN (?))
          ORDER BY hierarchy_entry_id", parent_ids, parent_ids
        )
        # TODO - Method here
        ...
        results.each do |result|
          next_level_of_parents << result["parent_id"]
          if results["hierarchy_entry_id"] != current_entry_id &&
            ! top_images.keys.nil?
            # this is a new entry so commit existing data before adding more
            bulk_insert_tmp_images(top_images, top_unpublished_images)
            he_ids << current_entry_id
            current_entry_id = results["hierarchy_entry_id"]
            top_images = {}
            top_unpublished_images = {}
            used_data_objects = []
          end
          next if used_data_objects.include?(results["data_object_id"])
          used_data_objects << results["data_object_id"]
          vetted_sort_order = vet_weights[results["vetted_id"]] || 5
          which = nil
          if results["visibility_id"] == Visibility.get_visible.id &&
            results["published"] == 1
            which = top_images
          else
            which = top_unpublished_images
          ...
          which[results["hierarchy_entry_id"]] ||= {}
          which[results["hierarchy_entry_id"]][vetted_sort_order] ||= {}
          which[results["hierarchy_entry_id"]][vetted_sort_order][results["data_rating"]] ||= {}
          which[results["hierarchy_entry_id"]][vetted_sort_order][results["data_rating"]][results["data_object_id"]] =
            "#{results["hierarchy_entry_id"]}, #{results["data_object_id"]}"
        end
        ...
        he_ids.in_groups_of(5000) do |group|
          connection.execute([
    ...
    def swap_tmp_table(table_name)
      count = begin
        connection.select_value("select count(*) from #{table_name}_tmp")
      rescue
        0
      end
      if count > 0
        connection.execute("RENAME TABLE #{table_name} TO #{table_name}_swap, "\
